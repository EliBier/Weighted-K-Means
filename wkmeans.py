...
class WKMeans():
    ...
    def _cluster_points(self):
        ...
        self.counts_per_cluster = np.bincount(self.cluster_indices, weights = self.counts_per_data_point)
        self.clusters = [[] for _ in range(self.K)]
        for index, x in zip(self.cluster_indices, self.X):
...
class KPlusPlus(WKMeans):
    ...
    # need to fix distance so that it calculates all of D2 at the same time instead of 1 at a time
    def _dist_from_centers(self):
        """Calculate the distance of each point to the closest centroids."""
        self.D2 = np.min(cdist(self.X, np.array(self.mu), 'sqeuclidean'), axis=1)
